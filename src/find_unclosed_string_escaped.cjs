...
for (let i = 0; i < lines.length; i++) {
    ...
    // Newline resets escape status
    escaped = false;
}

if (inSingle || inDouble) {
    console.log(`Unclosed ${inSingle ? 'single' : 'double'} quote detected with escape awareness!`);
    ...
    console.log('All quotes are balanced globally (escape-aware).');
}
